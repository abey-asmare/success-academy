...
import type { Answer, Exam, Question } from "@/prisma/app/generated/prisma/client"
    ...
    <div className="p-6 px-4 sm:px-6 lg:px-24">
        ...
        {!showResults && (
          <div
            className={cn(
              ...
              timeRemaining > 300 && timeRemaining <= 900 && "border-orange-200 bg-orange-50",
              timeRemaining > 900 && timeRemaining <= 1800 && "border-yellow-200 bg-yellow-50",
            )}
          >
            {timeRemaining <= 300 ? (
            ...
            <span className={cn("font-mono text-lg font-semibold", getTimerColor())}>{formatTime(timeRemaining)}</span>
          </div>
        )}
      </div>
      ...
      {isTimeUp && (
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-6">
          ...
          <p className="mt-1">The exam time has expired and your answers have been automatically submitted.</p>
        </div>
      )}
                  ...
                  {question.answers.map((answer, answerIndex) => (
                    <div
                      ...
                      className={cn(
                        ...
                        !showResults && !isTimeUp && "cursor-pointer hover:bg-gray-50",
                    ...
                    >
                      <div className="flex items-center gap-2 flex-wrap justify-between">
                        ...
                        {showResults && answer.isCorrect && <Check className="h-5 w-5 text-green-600 flex-shrink-0" />}
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="mt-8 px-4 min-h-[200px] transition-all duration-300">
          {!showResults ? (
            <div className="flex justify-center">
              <button
                ...
                className={cn(
                  ...
                  isTimeUp ? "bg-gray-400 text-gray-600 cursor-not-allowed" : "bg-sky-600 hover:bg-sky-700 text-white",
                )}
              >
            ...
            <div className="bg-gray-100 p-6 rounded-lg text-center max-w-2xl mx-auto">
              <h2 className="text-xl font-bold mb-2">{isTimeUp ? "Time's Up - Exam Results" : "Exam Results"}</h2>
              <p className="text-lg">
              ...
              {isTimeUp && (
                <p className="text-red-600 mt-2 text-sm">Exam was automatically submitted due to time expiration.</p>
              )}
              <button
                onClick={() => {
